...
export function generateProceduralLevel(levelNum: number): LevelConfig {
  ...
  // Grid scales +1 box every 2 levels:
  // Lv 1-2: 6x6, Lv 3-4: 7x7, Lv 5-6: 8x8, Lv 7-8: 9x9 ... up to 16x16 (and 18x18 for 1000+ infinite master levels)
  const grid = getGridDimensionForLevel(levelNum);

  // For 1000+ Infinity Master levels: higher snake density, longer winding snakes
  const minLen = isMasterInfinity ? 4 : 3;
  const maxLen = isMasterInfinity 
    ? Math.min(28, Math.floor(grid * 1.8))
    : Math.min(22, Math.floor(grid * 1.5));
...
export function getLevelConfig(levelId: number, isEndless: boolean = false): LevelConfig {
  if (!isEndless) {
    const predefined = LEVEL_MAP.get(levelId);
